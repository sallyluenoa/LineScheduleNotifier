...
class GoogleSheetsRepositoryImplTest {
    ...
    fun testFetchSheetData_cloudMode() {
        ...
        every { anyConstructed<GoogleSheetsCloudRepository>().fetchDataByRange("A1:B2") } returns expectedData

        val repository = GoogleSheetsRepositoryImpl(appConfig, secretProvider)
        val result = repository.fetchDataByRange("A1:B2")

        assertEquals(expectedData, result)
        verify(exactly = 1) { anyConstructed<GoogleSheetsCloudRepository>().fetchDataByRange("A1:B2") }
    }

    @Test
    fun testFetchDataByRange_mockMode() {
        val appConfig = mockk<AppConfig>()
        val secretProvider = mockk<SecretProvider>()
        ...
        every { anyConstructed<MockSheetsRepository>().fetchDataByRange("A1:B2") } returns expectedData

        val repository = GoogleSheetsRepositoryImpl(appConfig, secretProvider)
        val result = repository.fetchDataByRange("A1:B2")

        assertEquals(expectedData, result)
        verify(exactly = 1) { anyConstructed<MockSheetsRepository>().fetchDataByRange("A1:B2") }
    }
}
